...
namespace CGDev {
	...
	namespace wvk {
		...
		inline WvkStatus WvkCommandBuffer::reset(const VkCommandBufferResetFlags& flags) const noexcept {
			...
			auto _vk_result = m_create_info.wvk_logical_device_ptr->getWvkDispatchTable()->wvkResetCommandBuffer(m_vk_command_buffer, flags);
		...
		inline WvkStatus WvkCommandBuffer::begin(const VkCommandBufferBeginInfo& info) const noexcept {
			...
			auto _vk_result = m_create_info.wvk_logical_device_ptr->getWvkDispatchTable()->wvkBeginCommandBuffer(m_vk_command_buffer, &info);
			if (_vk_result != VK_SUCCESS) {
				switch (_vk_result) {
				case VK_ERROR_OUT_OF_DEVICE_MEMORY:
					_status.append("\nwvkResetCommandBuffer is VK_ERROR_OUT_OF_DEVICE_MEMORY.");
